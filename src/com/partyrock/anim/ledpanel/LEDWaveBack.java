 ...
 * 
 */
public class LEDWaveBack extends ElementAnimation {
    ...
    public LEDWaveBack(LightMaster master, int startTime, ArrayList<ElementController> elementList, double duration) {
        ...
        for (ElementController controller : getElements()) {
            ...
            if (fadedcol < colon) {
            	for (int c = fadedcol + 1; c <= colon && c < panel.getPanelWidth(); c++) {
            		if (c < 4) {
            			panel.setColor(7 - c, 15 - c, pink);
                		panel.setColor(8 - c, 15 - c, pink);
            		}
            		else if (c < 12) {
            			panel.setColor(c + 1, 15 - c, pink);
                		panel.setColor(c + 2, 15 - c, pink);
            		}
            		else {
            			panel.setColor(12 * 2 - c - 1, 15 - c, pink);
            			panel.setColor(13 * 2 - c - 2, 15 - c, pink);
            		}
            	}
            }
        }
